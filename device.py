...
class DeviceException(Exception):
    '''Base exception for scantool errors'''

class MultilineException(DeviceException):
    '''Raised when multiline data cannot be assembled'''
...
class ELM327:
    ...
    def set_protocol(self, protocol: int):
        ...
        if 'OK' not in lines: raise DeviceException('failed to set protocol')
    ...
    def send_command(self, cmd: str, prompt_char=b'>') -> list[str]:
        ...
        # read from serial port until prompt char is received or timeout
        buf = self.port.read_until(prompt_char)
        logger.debug(f'RX: {buf}')
        
        if len(buf) == 0: raise DeviceException('no data received')
    ...
    def set_header(self, header: bytes):
        ...
        if 'OK' not in lines: raise DeviceException('failed to set header')
    ...
    def send_message(self, message: VPWMessage) -> VPWMessage:
        ...
        for line in response:
            try:
                ...
                warnings.warn(f'non-hex data received: {line}')
                continue

            data_frames.append(frame)

        if len(data_frames) == 0: raise DeviceException('no data received')

        data_start = 4 + len(message.request) # starting byte of response data
        ...
        if len(data_frames) == 1:
            ...
            for frame in data_frames:
                ...
                # verify multiline response lines have matching initial bytes
                if not frame.startswith(data_frames[0][:data_start]):
                    raise MultilineException('invalid index')
                
                # verify first data byte matches expected index
